...
if let foundationObject = try JSONSerialization.jsonObject(with: jsonData) as? [String: Any]{
  if let dog = Dog(from: foundationObject) {
...
if let foundationObject = try JSONSerialization.jsonObject(with: jsonArrData) as? [[String: Any]]{
  foundationObject.forEach{
    if let dog = Dog(from: $0) {
      print("Serialization :", dog)
    }
  }
}
// JSONDecoder
if let dog = try? JSONDecoder().decode([Dog].self, from: jsonArrData) {
  dog.forEach{
    print("Decoder :", $0)
  }
}
...
if let foundationObject = try JSONSerialization.jsonObject(with: jsonDictData) as? [String:[[String:Any]]], let data = foundationObject["data"] as? [[String:Any]]{
  data.compactMap{
    Dog(from: $0)
  }.forEach{
    print("Serialization :", $0)
  }
}
// JSONDecoder
if let dog = try? JSONDecoder().decode([String:[Dog]].self, from: jsonDictData) {
//  dog["data"]?.forEach{
//    print("Decoder :", $0)
//  }
  dog.values.forEach{
